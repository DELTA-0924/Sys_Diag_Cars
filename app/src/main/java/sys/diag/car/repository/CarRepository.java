...
public class CarRepository {
    ...
    public void syncData(String token , MutableLiveData<Result<ResponseContact>> liveResponse) {
                ...
                call.enqueue(new Callback<ResponseContact>() {
                    ...
                    public void onResponse(Call<ResponseContact> call, Response<ResponseContact> response) {
                        if (response.isSuccessful()) {
                            setSyncFlag(carsId);
                            liveResponse.postValue(Result.success(
                                    new ResponseContact("Код : " + response.code(), "Успешно синхронизировано")));

                        } else {
                            liveResponse.postValue(Result.error("Ошибка синхронизации " + response.code(), null));
                        }
                    }
    ...
    public void loadData(String token, MutableLiveData<Result<ResponseContact>> cars){
    ...
    private void saveAll(List<CarResponse>carsResponse){
        ...
        executor.execute(()->carDAO.insertAll(carEntities));
